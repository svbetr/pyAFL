...
class Player(object):
    ...
    def _get_player_url(self):
        ...
        return f"{config.AFLTABLES_STATS_BASE_URL}stats/{url_list[0].attrs.get("href")}"
    ...
    def _get_bio_info(self, b_tags):
        for bio in b_tags:
            ...
            if re.sub(r"[\n\t\s]*", "", bio.get_text()) == "Born:":
                ...
                if not date_born:
                    self.metadata["born"] = None
                    continue

                timestamp = datetime.strptime(date_born, "%d-%b-%Y").strftime(
                    "%d-%b-%Y"
                )
            ...
            if re.sub(r"[\n\t\s]*", "", bio.get_text()) == "Debut:":
                ...
                if not debut or self.metadata["born"] == None:
                    self.metadata["debut"] = None
                    continue
            ...
            if re.sub(r"[\n\t\s]*", "", bio.get_text()) == "Last:":
                ...
                if not last or self.metadata["born"] == None:
                    self.metadata["last"] = None
                    continue
    ...
    def get_player_stats(self):
        ...
        all_dfs = pd.read_html(StringIO(str(self._stat_html)))
        season_dfs = pd.read_html(
            StringIO(str(self._stat_html)), match=r"[A-Za-z]* - [0-9]{4}"
        )
